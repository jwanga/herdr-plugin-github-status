...
pub fn parse_github_remote(url: &str) -> Option<(String, String)> {
    ...
    let name = name.strip_suffix(".git").unwrap_or(name).trim_end_matches('/');
...
pub fn detect(cwd: &str) -> Option<RepoRef> {
    ...
    Some(RepoRef { owner, name, branch, root })
}
...
mod tests {
    ...
    fn parses_remote_forms() {
        let want = Some(("jwanga".to_string(), "herdr-plugin-github-status".to_string()));
        for url in [
    ...
    fn prefers_origin_then_any_github_remote() {
        let remotes = vec![
            ...
            ("origin".to_string(), "git@github.com:me/mine.git".to_string()),
        ];
        assert_eq!(pick_remote(&remotes), Some(("me".into(), "mine".into())));
        let remotes = vec![
            ("origin".to_string(), "git@gitlab.com:me/mine.git".to_string()),
